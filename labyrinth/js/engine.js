...
import { initInputHandler, updateInput, isAutoPlay as inputIsAutoPlay, setAutoPlay as inputSetAutoPlay } from './input-handler.js';
import { saveGame, loadGame, applySave, hasSave, startAutoSave, deleteSave, syncCloudSave } from './save-system.js';
import { initSettingsMenu, toggleSettingsMenu, isSettingsOpen, getSetting, loadSettings } from './settings-menu.js';
import { initSteamIntegration, checkAchievements, onMonsterKill, onPlayerDeath, onFloorCleared, onExtraction, onEchoKilled, onBiomeVisited, updateRichPresence } from './steam-integration.js';
import { initTutorial, updateTutorial, setTutorialEnabled } from './tutorial.js';
...
export function init() {
  ...
  initInputHandler(canvas, {
    movePlayer: (dx, dy) => {
      if (!isSettingsOpen()) movePlayer(dx, dy);
    },
    onInteract: () => {
      // Attack adjacent enemy or interact with environment
      const p = Game.player;
      const nx = p.x + DX[p.dir], ny = p.y + DY[p.dir];
      const mon = Game.monsterAt(nx, ny);
      if (mon) movePlayer(DX[p.dir], DY[p.dir]);
    },
    onInventory: () => {
      // Toggle telemetry (acts as inventory display for now)
    ...
    onExtract: () => {
      const p = Game.player;
      if (Game.tiles[p.y]?.[p.x] === T_STAIRS && !Game.bossAlive && !extractor.extracting) {
        extractor.startExtract(p.x, p.y);
        Game.addLog('EXTRACTING...', '#ffaa00');
      }
    },
  ...
  initSettingsMenu({
    onResume: () => { /* game resumes automatically */ },
    onQuit: () => {
      // Save before quit
      saveGame(Game);
      if (window.labyrinth && window.labyrinth.isElectron) {
        window.close();
      }
    },
  ...
  // ─── Tutorial ───
  initTutorial();
  if (savedSettings.showTutorial === false) setTutorialEnabled(false);

  // ─── Save/Load System ───
  if (hasSave()) {
    const saveData = loadGame();
    if (saveData) {
      applySave(saveData, Game);
      // Regenerate floor at saved depth
      generateNewFloor();
      Game.addLog('Save loaded — Depth ' + Game.depth, '#00ff41');
    }
  }
  // Sync cloud save if available
  syncCloudSave();

  // Start auto-save (every 60s)
  startAutoSave(() => Game);
...
function gameLoop(timestamp) {
  ...
  // Skip game logic when settings menu is open
  if (isSettingsOpen()) {
    if (postFX) postFX.render(gameTime);
    else renderer.render(scene, camera);
    return;
  }

  // Player input (takes priority over AI)
  updateInput(dt);

  // AI auto-play (only when player is not controlling)
  if (inputIsAutoPlay()) aiTick(dt);
  ...
  if (extractResult) {
    if (extractResult.type === 'extracting') {
      ...
      onFloorCleared(Game.depth, Game);
      onExtraction(Game);
      generateNewFloor();
  ...
  if (currentMood !== Game.mood && dungeonGroup) {
    ...
    onBiomeVisited(Game.biome.name || currentMood);
  }
  ...
  // Periodic achievement check (every ~60 frames)
  if (Math.floor(gameTime * 20) % 60 === 0) {
