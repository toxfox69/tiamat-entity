# TIAMAT Hive Step 1B — Training Data Logger

Run this on the droplet after 1A is confirmed working.
Starts collecting self-distillation training data. Every cycle from now on
generates data for TIAMAT's own future model.

---

```
Create /root/entity/src/agent/training_logger.py (or .ts if the codebase is
TypeScript — adapt accordingly):

This module logs every cycle's input/output as fine-tuning training data.
The format must be compatible with standard fine-tuning APIs (OpenAI format,
Together.ai, HuggingFace Axolotl).
...
Now integrate this into the main agent loop. Find where each cycle completes —
after the model returns a response and tools have been called. Add a call to
log_training_example() with all the relevant data from that cycle.

This should be a non-blocking, fire-and-forget append. It's just writing one
line to a JSONL file — essentially zero overhead.

Also add a /training-stats route to /root/summarize_api.py:
...
Restart gunicorn after adding the route: pkill -HUP gunicorn

Test:
1. Let TIAMAT run 5-10 cycles
2. Check: ls -la /root/.automaton/training_data/
3. Check: wc -l /root/.automaton/training_data/cycles_2026-02.jsonl
4. Check: curl https://tiamat.live/training-stats

Show me the results.

Commit: "feat: add training data logger for self-distillation pipeline"
Push to main.
```
