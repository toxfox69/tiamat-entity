...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/fig1_cost_trajectory.pdf}
\caption{Cost per cycle over 21,111 autonomous cycles. Blue line: 200-cycle rolling average. Red dashed lines: quartile means. Green line: linear trend. The 40\% decline from Q2 peak to Q4 reflects autonomous cost optimization through model routing refinement.}
...
Xu et al.\ do not report per-cycle costs, but their exclusive use of frontier models (Sonnet 4.5 and Opus 4.5) suggests significantly higher per-cycle costs than TIAMAT's blended rate. TIAMAT achieves continuous operation by being selective about when to invoke expensive inference---reserving frontier models for strategic bursts while routing the majority of cycles through cheaper alternatives. Figure~\ref{fig:model_routing} shows the cost distribution across model categories.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{figures/fig3_model_routing.pdf}
\caption{Inference cost distribution by model category. Claude Code CLI sessions account for 66.9\% of total spend despite representing multi-turn agentic work. Direct API calls (Sonnet + Haiku + cascade models) provide the single-turn inference at lower per-call costs.}
\label{fig:model_routing}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.65\textwidth]{figures/fig2_memory_funnel.pdf}
\caption{Three-tier memory compression pipeline. 6,346 raw observations are compressed through 3,190 autonomous consolidation runs into 1,533 distilled facts (4.14:1 overall compression). L2 uses Jaccard-similarity clustering; L3 extracts core knowledge with confidence scoring.}
\label{fig:memory_funnel}
\end{figure}
...
To address whether compression preserves useful information, we conducted a heuristic quality evaluation across all three memory tiers. Table~\ref{tab:l3_quality} presents the L3 core knowledge quality breakdown.

\begin{table}[h]
\centering
\caption{L3 core knowledge quality evaluation (N=1,533)}
\label{tab:l3_quality}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Quality Category} & \textbf{Count} & \textbf{\%} \\
\midrule
Accurate \& Actionable & 867 & 56.6\% \\
Accurate but Trivial & 335 & 21.8\% \\
Stale / Outdated & 41 & 2.7\% \\
Garbage / Noise & 290 & 18.9\% \\
\midrule
\textbf{Effective Quality Rate} & \textbf{1,202} & \textbf{78.4\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig4_l3_quality.pdf}
\caption{L3 core knowledge quality evaluation. Heuristic classification of all 1,533 distilled facts. 56.6\% are accurate and actionable; 18.9\% represent noise that the compression pipeline should have filtered.}
\label{fig:l3_quality}
\end{figure}

Of 1,533 distilled L3 facts, 56.6\% (867) are accurate and actionable---containing specific technical knowledge, strategic insights, or operational lessons useful for future decision-making (e.g., ``SendGrid API requires verified sender identity,'' ``Dev.to rate limit is 1 article per hour''). An additional 21.8\% (335) are accurate but trivially obvious. The garbage rate of 18.9\% (290) represents logging artifacts and tool execution metadata that the compression pipeline incorrectly promoted to permanent knowledge. The staleness rate is low (2.7\%), suggesting that the consolidation system is effective at producing durable facts rather than ephemeral observations.

L2 compression fidelity was evaluated on 20 randomly sampled summaries, yielding a mean preservation score of 3.05/5.0---indicating that compressed summaries retain approximately half of the key information from their source memories. This moderate score reflects a deliberate trade-off: aggressive compression reduces token cost for memory retrieval at the expense of some information loss.

The most striking finding concerns recall utilization. Of 6,346 L1 memories, 99.9\% have never been recalled (recall count = 0). This suggests a fundamental inefficiency: the agent invests cycles in storing memories that are never subsequently accessed. The smart recall system---designed to search L3 first, then L2, then L1---may be too effective at the L3 level, rarely needing to access lower tiers. Alternatively, the memory system may be over-producing observations relative to the agent's actual retrieval needs. Optimizing memory production to reduce storage of never-recalled observations represents a clear efficiency target.

Knowledge graph triples showed 93.3\% accuracy (28 of 30 sampled), with only 1 outdated and 1 garbage entry, indicating that the triple extraction pipeline applies more rigorous quality filtering than the L3 fact pipeline. No near-duplicate L3 facts were detected in a 200-entry sample (Jaccard similarity threshold 0.8), confirming that the deduplication stage of consolidation is effective.
...
\begin{itemize}
    ...
    \item \textbf{Memory quality partially evaluated.} Our heuristic quality evaluation (Section 5.3) provides an initial assessment of memory fidelity, but does not measure downstream impact---whether higher-quality memories produce measurably better decisions in subsequent cycles. The 99.9\% never-recalled rate for L1 memories suggests the memory system over-produces observations relative to retrieval needs.
