...
def generate_segment(mood='processing', seed=None, duration=45.0, sr=SR):
    ...
    # ── Mix ──
    mix = (_fit(sub) * 0.4 +
           _fit(bass) * profile['bass_vol'] +
           _fit(pad) * profile['pad_vol'] +
           _fit(arp) * profile['arp_vol'] +
           _fit(lead) * profile['lead_vol'] +
           _fit(drums) * profile['drum_vol'])

    # ── Master chain ──
    mix = reverb_conv(mix, sr, profile['reverb_mix'], decay_time=0.45)
    ...
    # Stereo
    stereo = stereo_field(mix, delay_ms=11, width=0.25, sr=sr)
